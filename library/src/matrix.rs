...
#[derive(Debug, Clone)]
struct MatRowIterator<'a, T> {
    mat: &'a Mat<T>,
    curr: usize,
}

impl<'a, T> Iterator for MatRowIterator<'a, T> {
    type Item = &'a [T];

    fn next(&mut self) -> Option<Self::Item> {
        if self.curr < self.mat.rows {
            let row = &self.mat[self.curr];
            self.curr += 1;
            Some(row)
        } else {
            None
...
#[derive(Debug, Clone)]
struct MatRowIteratorMut<'a, T> {
    mat: &'a mut Mat<T>,
    curr: usize,
}

impl<'a, T> Iterator for MatRowIteratorMut<'a, T> {
    type Item = &'a mut [T];

    fn next(&mut self) -> Option<Self::Item> {
        if self.curr < self.mat.rows {
            let row = self.mat.row_mut(self.curr);
            self.curr += 1;
            Some(row)
        } else {
            None
